 ...
 */
static volatile queue * fromDir;
static volatile queue * toDir;
...
intersection_sync_init(void)
{
  ...
  toDir = q_create(100);
  if(toDir == NULL){
    panic("could not create toDir queue\n");
  }
  fromDir = q_create(100);
  if(fromDir == NULL){
    panic("could not create fromDir queue\n");
  }
  numCars=0;
...
bool canEnter(Direction origin, Direction destination){
  for (int i = 0 ; i < numCars; i++){
    ...
    if(  !(fromDir == (int)origin  && toDir[i] == (int)destination)  // same vehicle 
